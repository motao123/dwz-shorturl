...
- 自定义短码：生成时可指定 6-8 位短码（自动校验格式与占用）
- 短链有效期：支持 1 天 / 7 天 / 30 天 / 1 年 / 永久，过期跳转返回 410
- 二维码：生成结果弹窗内一键生成短链二维码（纯前端生成，短链不离开浏览器）
...
- `custom`（可选）：自定义短码，6-8 位，仅含 `a-z` 与 `0-5`；已占用或格式错误返回对应错误码。
- `expire`（可选）：有效期天数，支持 `0`(永久)/`1`/`7`/`30`/`365`；过期后跳转返回 HTTP 410。
- 超过限流频率返回 `{"code":0,"msg":"请求过于频繁，请稍后再试","result":10005}`，HTTP 429。

### 伪静态（短链跳转）
- Apache：仓库已附 `.htaccess`，将 `/<短码>` 重写到 `do.php?uid=<短码>`，无需额外配置。
- Nginx：参考下方配置（`do.php` 同时兼容直接 `?uid=` 访问，伪静态仅为美化）。
...
| 10005 | 请求过于频繁 |
| 10006 | 自定义短码格式错误（需 6-8 位 a-z/0-5） |
| 10007 | 自定义短码已被占用 |
