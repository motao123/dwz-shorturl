...
CREATE TABLE IF NOT EXISTS `short_urls` (
  ...
  `member_id`   BIGINT UNSIGNED NULL COMMENT 'public member ID, NULL=anonymous/admin-created',
  `source`      VARCHAR(16)    NOT NULL DEFAULT 'web' COMMENT 'web/api/batch/admin',
  `ip`          VARCHAR(45)    NULL COMMENT 'creator IP',
  `reminder_sent_at` DATETIME(3) NULL COMMENT 'last expiry reminder sent at',
  `created_at`  DATETIME(3)    NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `updated_at`  DATETIME(3)    NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  `deleted_at`  DATETIME(3)    NULL,
...
('audit', 'update', 'Review violation reports'),
('audit', 'delete', 'Delete violation reports'),
